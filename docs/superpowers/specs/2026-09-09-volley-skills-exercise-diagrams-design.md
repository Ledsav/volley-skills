...
| `pole` | — | |
| `line` | `points` array of `{x,y}`, length 2–12; `style` `"solid" \| "dashed"`; `thickness` number 1–4 | drags as a whole shape (all points shift together) or by a single endpoint dot (`moveEndpoint`) |
...
- **Three render passes:**
  ...
  3. When `interactive && selectedId`: a selection outline around the selected
     ...
     Line/arrow selection instead shows draggable endpoint markers (grabbing one
     dispatches `moveEndpoint` — see §6.2).
...
- Dragging a token dispatches `translateItem` (a per-move `{dx,dy}` delta, snapped
  ...
  (every point / `from`+`to` shift by the same delta); dragging one of its
  endpoint dots dispatches `moveEndpoint` (the absolute court position of that
  endpoint, snapped when Snap is on — arrow index 0→`from`, 1→`to`; line →
  `points[index]`), reshaping the segment's length and angle at once.
...
- Line/arrow endpoints are individually draggable (grab an endpoint dot →
  `moveEndpoint`, reshaping length + angle); the whole shape can still be moved as
  a unit via `translateItem`. Numeric per-field endpoint editors are still
  deferred (next bullet).
- The properties panel does not yet expose every type-specific field
  ...
  `arrow.curved`/`head`, `line`/`arrow` points as numbers); those values take their
